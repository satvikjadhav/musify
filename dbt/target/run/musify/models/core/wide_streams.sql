  ...
  create or replace view `musify-354416`.`musify_prod`.`wide_streams`
  OPTIONS()
  as 
...
FROM
    `musify-354416`.`musify_prod`.`fact_streams`
JOIN
    `musify-354416`.`musify_prod`.`dim_users` ON fact_streams.userKey = dim_users.userKey
JOIN
    `musify-354416`.`musify_prod`.`dim_songs` ON fact_streams.songKey = dim_songs.songKey
JOIN
    `musify-354416`.`musify_prod`.`dim_location` ON fact_streams.locationKey = dim_location.locationKey
JOIN
    `musify-354416`.`musify_prod`.`dim_datetime` ON fact_streams.dateKey = dim_datetime.dateKey
JOIN
    `musify-354416`.`musify_prod`.`dim_artists` ON fact_streams.artistKey = dim_artists.artistKey;
