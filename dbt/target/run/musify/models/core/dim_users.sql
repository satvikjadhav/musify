  ...
  create or replace table `musify-354416`.`musify_prod`.`dim_users`
...
SELECT to_hex(md5(cast(coalesce(cast(userId as 
    string
), '') || '-' || coalesce(cast(rowActivationDate as 
    string
), '') || '-' || coalesce(cast(level as 
    string
), '') as 
    string
))) as userKey, *
