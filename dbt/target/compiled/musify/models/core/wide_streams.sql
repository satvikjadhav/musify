...
FROM
    `musify-354416`.`musify_stg`.`fact_streams`
JOIN
    `musify-354416`.`musify_stg`.`dim_users` ON fact_streams.userKey = dim_users.userKey
JOIN
    `musify-354416`.`musify_stg`.`dim_songs` ON fact_streams.songKey = dim_songs.songKey
JOIN
    `musify-354416`.`musify_stg`.`dim_location` ON fact_streams.locationKey = dim_location.locationKey
JOIN
    `musify-354416`.`musify_stg`.`dim_datetime` ON fact_streams.dateKey = dim_datetime.dateKey
JOIN
    `musify-354416`.`musify_stg`.`dim_artists` ON fact_streams.artistKey = dim_artists.artistKey